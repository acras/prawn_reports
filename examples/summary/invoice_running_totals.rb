...
class InvoiceSummary < PrawnReport::SummaryBand
  ...
  def internal_draw
    ...
    begin
      ...
      report.x = 200
      report.text(report.totals['quantity'].to_s, 30, :font_size => 12)
      report.text(report.totals['unit_price'].to_s, 30, :font_size => 12)
    ensure
...
class InvoiceReport < PrawnReport::SimpleListing
  ...
  def initialize(params = {})
    params.merge!({:running_totals => ['quantity', 'unit_price']})
    super(params)
