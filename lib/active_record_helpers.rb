...
class ActiveRecordHashSerializer
  ...
  def serialize
    @contents = {}
    @contents.merge!(serialize_root_values(@params))

    if @obj.is_a? ActiveRecord::Base
      @contents.merge!(serialize_record(@obj, false, @params)) #asnotarray
    elsif @obj.is_a? Array
      @contents['items'] = []
      @obj.each do |item|
        @contents['items'] <<  serialize_record(item, true, @params)  #asarray
      end
    end
  ...
  def serialize_root_values(params)
    r = {}
    params[:root_values] ||= {}
    params[:root_values].each_pair { |k,v| r[k.to_s] = serialize_string(v)  }
    r
  end
  ...
  def serialize_record(rec, as_array, params = {})
    ...
    to_serialize.each_pair do |k, v|
      r[k] = serialize_string(v)
    end

    if rec.is_a? ActiveRecord::Base
      r.merge!(serialize_belongs_tos(rec, first_line, params))
      r.merge!(serialize_has_manys(rec, first_line, params))
      r.merge!(serialize_methods(rec, first_line, params))
    end
  ...
  def serialize_belongs_tos(rec, first_line, params)
    ...
    if params[:include_all_belongs_to] == true
      rec.class.reflect_on_all_associations(:belongs_to).collect do |a|
        r[a.name.to_s] = serialize_record(rec.send(a.name), false,
          {:include_all_belongs_to => params[:include_all_belongs_to]}) if rec.send(a.name)
      end
    else
      params[:included_belongs_to] ||= {}
      params[:included_belongs_to].each_pair do |k,v|
        ...
        if type == :fields
          v[:fields].each do |f|
            val = master_rec ? master_rec.send(f) : nil
            r[k.to_s + '_' + f.to_s] = serialize_string(val)
          end
        elsif type == :record
          r[k.to_s] = serialize_record(master_rec, false, v[:params]) if master_rec #as_not_array
        end
      end
    end
    r
  ...
  def serialize_has_manys(rec, first_line, params)
    ...
    params[:included_has_many] ||= {}
    params[:included_has_many].each_pair do |k,v|
      r[k.to_s] = serialize_has_many(rec.send(k), v)
    end
    r
  end
...
module PrawnReportActiveRecord
  ...
  def pr_serialize(params = {})
    a = ActiveRecordHashSerializer.new(self, params)
